...
type
  TFrmDefinicje = class(TForm)
    ...
    RzTbshtMiejscowości: TRzTabSheet;
    ...
    procedure RzTbshtMiejscowościShow(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Wypełnij wymagane pola');
    end
    else
      ZapiszKierowce;
  end;
  ...
  try
    ...
    begin
      ...
      try     //do zm. historia przypisuje legende + zawartosc editow
        historia := 'Dodanie nowej miejscowości ' + #13#10;
        ...
        ShowMessage('Błąd! Nie dodano wpisu w historii. Sprawdź dane!');
      end;
    end;
    //koniec historia
    ...
    DataModule1.ibTransHistoria.Rollback;
    ShowMessage('Błąd nie udało się utworzyć nowej miejscowości ');
  end;
...
begin
  ...
  {  with DataModule1.ibQryTemp, SQL do                    //po dodaniu w oknie wczesniejszym ustawiamy sie na nowym instruktorze
    ...
    DataModule1.ibTransHistoria.Rollback;
    ShowMessage('Błąd nie udało się utworzyć nowego kierowcy ');
  end;
  rzEdtMarka.Text := '';
  rzEdtTyp.Text := '';
  rzEdtNrRej.Text := '';
...
var
  generator: Integer;  //potrzebna do ustawienia się na nowym rekordzie
...
begin
case Application.MessageBox('Jeżeli anulujesz, okno zostanie zamknięte, ' + #13#10 + 'żadne zmiany nie zostaną zapisane.', 'Caption', MB_YESNO + MB_ICONWARNING) of
    IDYES:
      begin
        Close;
      end;
...
begin
  ctgryBtns1.Categories[0].Items[0].Caption := '[F9]  Dodaj kierowcę';
...
procedure TFrmDefinicje.RzTbshtMiejscowościShow(Sender: TObject);
begin
  ctgryBtns1.Categories[0].Items[0].Caption := '[F9]  Dodaj miejscowość';
